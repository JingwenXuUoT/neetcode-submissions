class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        n = len(position)
        car_fleer = n # n is when no car catch up with the car ahead, and each car reaches the destination individually.
        # can catch up before or at destination:
        # (target-front_position) / front_speed >= (target-back_position) / back_speed, if true, car_fleet -= 1
